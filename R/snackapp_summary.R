...
#'
#' @examples
snackapp_summary <- function(folder_path = file.path(getwd(), "data"), csv = FALSE, r_object = TRUE, output_path = file.path(getwd(), "summary")) {
  ...
  if (csv == TRUE) {
    write.csv(summary, file = paste(output_path, "/", "participant_summary.csv", sep = ""))
  }
  if (r_object == TRUE) {
